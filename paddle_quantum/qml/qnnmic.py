...
class ImageDataset(Dataset):
    ...
    def __init__(
        ...
        if task == "train":
            ...
            data_label = list(npz_file['train_labels'])
            data_set = np.array(data_set)
            data_label = np.array(data_label)
            
        elif task == "val":
            data_set = npz_file['val_images']
            data_label = npz_file['val_labels']
        elif task == 'test':
            data_set = npz_file['test_images']
            data_label = npz_file['test_labels']
        else:
            raise ValueError

        data = np.reshape(data_set, (data_set.shape[0], data_set.shape[1] * data_set.shape[2]))

        # use sampling to avoid imbalance classification
        if task == "train":
            smote_tomek = SMOTETomek(random_state=0)
            data, data_label = smote_tomek.fit_resample(data,data_label)
...
class QNNMIC(paddle.nn.Layer):
    r"""
    The class of the medical image classification using quantum neural networks (QNN).

    Args:
        num_qubits: The number of qubits of quantum circuit in each layer. Defaults to ``[8, 8]``.
        num_depths: The number of depths of quantum circuit in each layer.  Defaults to ``[2, 2]``
        observables: The observables of quantum circuit in each layer. Defaults to ``[['Z0','Z1','Z2','Z3'], ['X0','X1','X2','X3']]``.
    """
    def __init__(self, num_qubits: List[int], num_depths: List[int], observables: List) -> None:
        super(QNNMIC, self).__init__()
    ...
    def forward(self, batch_input: List[paddle.Tensor]) -> paddle.Tensor:
        ...
        Returns:
            Return the output of the model. It's shape is :math:`(\text{batch_size}, -1)` .
        """
        features = []
...
def train(
        ...
        dataset: str = 'pneumoniamnist', saved_dir: str = './',
    ...
    Args:
        ...
        num_qubits: The number of qubits of quantum circuit in each layer.
        num_depths: The depth of quantum circuit in each layer.
        observables: The observables of quantum circuit in each layer.
        batch_size: The size of the batch samplers. Defaults to ``20`` .
        num_epochs: The number of epochs to train the model. Defaults to ``4`` .
        learning_rate: The learning rate used to update the parameters. Defaults to ``0.1``.
        dataset: The path of the dataset. It defaults to ``pneumoniamnist``.
        saved_dir: The path used to save logs. Defaults to ``./``.
        ...
        num_train: The number of the data in the training dataset. Defaults to ``-1`` , which will use all training data.
        num_val: The number of the data in the validation dataset. Defaults to ``-1`` , which will use all validation data.
        num_test: The number of the data in the test dataset. Defaults to ``-1`` , which will use all test data.
        early_stopping: Number of the iterations with no improvement after which training will be stopped. Defaults to ``1000``.
        num_workers: The number of subprocess to load data, 0 for no subprocess used and loading data in main process. Defaults to 0.
    """
    if saved_dir[-1] != '/':
        saved_dir += '/'
    ...
    train_dataset = ImageDataset(file_path=dataset, num_samples=num_train, task="train")
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    test_dataset = ImageDataset(file_path=dataset, num_samples=num_test, task='test', 
                                pca=train_dataset.pca, scaler=train_dataset.scaler, centering=train_dataset.centering)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    if using_validation:
        val_dataset = ImageDataset(file_path=dataset, num_samples=num_val, task='val',
                                pca=train_dataset.pca, scaler=train_dataset.scaler, centering=train_dataset.centering)
        val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    paddle.seed(0)
    model = QNNMIC(num_qubits, num_depths, observables)
    model.train()
...
def inference(
    ...
    train_dataset = ImageDataset(file_path="pneumoniamnist", num_samples=num_samples, task="train")
    dataset = ImageDataset(file_path=image_path, num_samples=num_samples, task='test', pca=train_dataset.pca, scaler=train_dataset.scaler, centering=train_dataset.centering)

    model.eval()
    prob = model(paddle.to_tensor(dataset.image_list))
    prediction = [int(1-paddle.argmax(i)) for i in prob]
    label = np.argmin(dataset.label_list[:num_samples], axis=1)

    msg = f"Finish Inferencing"
    logging.info(msg)
    
    return prediction, prob.tolist(), label.tolist()
